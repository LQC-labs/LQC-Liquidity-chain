...
As of **September 6, 2026**, the repository contains an unaudited, testnet-oriented DEX and Router 2.0 implementation. The current branch compiles 18 Solidity source files and passes 45 local automated tests. It includes a constant-product AMM, route quoter, allowlisted external-DEX adapters, native-BNB routes, two-way ERC-20 split execution, token allowlisting, per-trade and daily input caps, function-level swap pausing, a governance timelock designed for multisig administration, an off-chain route optimizer, and a responsive wallet-connected trading interface with optional timestamped OHLCV market data.
