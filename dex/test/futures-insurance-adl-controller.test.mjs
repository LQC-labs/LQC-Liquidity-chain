...
const profitableShort = Object.freeze({
  id: 'short-1', side: 'SHORT', quantity: 1, entryPrice: 120, markPrice: 100, collateral: 20
});

const profitableLong = Object.freeze({
  id: 'long-1', side: 'LONG', quantity: 1, entryPrice: 80, markPrice: 100, collateral: 20
});

describe('Futures insurance and ADL orchestration', () => {
  test('fully covers liquidation loss without invoking ADL', () => {
    const controller = createDemoInsuranceAdlController({ initialFund: createInsuranceFund({ balance: 100 }) });
  ...
  test('passes only residual bad debt to ADL after insurance is exhausted', () => {
    const controller = createDemoInsuranceAdlController({ initialFund: createInsuranceFund({ balance: 30 }) });
  ...
  test('respects per-event insurance coverage cap before ADL', () => {
    const controller = createDemoInsuranceAdlController({
      initialFund: createInsuranceFund({ balance: 100, maxCoveragePerEvent: 25 })
    });
  ...
  test('keeps explicit residual debt when ADL liquidity is insufficient', () => {
    const controller = createDemoInsuranceAdlController({ initialFund: createInsuranceFund({ balance: 0 }) });
