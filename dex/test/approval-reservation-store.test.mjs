...
class RacingStorage extends MemoryStorage{arm(value,remainingReads=2){this.remainingReads=remainingReads;this.racingValue=value}getItem(key){if(this.remainingReads){this.remainingReads--;if(this.remainingReads===0)this.values.set(key,this.racingValue)}return super.getItem(key)}}
...
describe('LQC approval signing reservation store',function(){
  ...
  it('never overwrites a reservation created by another tab after the empty-state read',function(){const storage=new RacingStorage(),store=make(storage),newer='other-tab-reservation';storage.arm(newer);assert.throws(()=>store.reserve(binding,approval),/Conflict/);assert.equal(storage.getItem(store.key),newer)});
  it('never overwrites a reservation created after the first empty identity check',function(){const storage=new RacingStorage(),store=make(storage),newer='late-other-tab-reservation';storage.arm(newer,3);assert.throws(()=>store.reserve(binding,approval),/Conflict/);assert.equal(storage.getItem(store.key),newer)});
  it('never deletes a reservation changed by another tab after the identity read',function(){const storage=new RacingStorage(),store=make(storage),serialized=store.reserve(binding,approval),newer='newer-other-tab-reservation';storage.arm(newer);assert.equal(store.clear(serialized),false);assert.equal(storage.getItem(store.key),newer)});
