...
describe("Router 2.0 Proof Gateway operational monitor page",function(){
  ...
  it("records a block-number-and-hash-bound operational snapshot",function(){assert.match(js,/provider\.getBlock\("latest"\)/);assert.match(js,/blockHash/);assert.match(js,/solidityPackedKeccak256/);assert.match(js,/localStorage\.setItem\(MONITOR_KEY/);assert.match(html,/조회 블록 해시/);});
  ...
  it("seals a public V2 PASS or FAIL report with a deterministic hash",function(){assert.match(js,/function sealReport\(core\)/);assert.match(js,/LQC_PROOF_GATEWAY_MONITOR_V2/);assert.match(js,/status:"PASS"/);assert.match(js,/status:"FAIL"/);assert.match(js,/keccak256\(ethers\.toUtf8Bytes\(canonical\)\)/);assert.match(html,/공유용 Report Hash/);});
