...
it("lets only IntentHub consume the exact selected solver and quote once",function(){assert.match(s,/modifier onlyIntentHub/);assert.match(s,/if\(a\.solver!=solver\)revert WrongSolver/);assert.match(s,/if\(a\.quoteHash!=quoteHash\)/);assert.match(s,/if\(a\.consumed\)revert AssignmentAlreadyConsumed/);});
});
