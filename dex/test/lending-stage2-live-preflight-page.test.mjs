...
describe("LQC Lending Stage-2 live preflight page",function(){
  it("is explicitly read-only and pins two independent public RPCs",function(){assert.match(html,/읽기 전용/);assert.match(html,/지갑 연결·서명·배포·거래·가스비가 없습니다/);assert.match(js,/bsc-testnet-rpc\.publicnode\.com/);assert.match(js,/data-seed-prebsc-1-s1\.bnbchain\.org/);assert.match(html,/주소 복사나 입력이 필요 없습니다/);assert.doesNotMatch(js,/window\.ethereum|eth_sendTransaction|eth_requestAccounts/)});
  ...
  it("forces mobile browsers to load the corrected automatic-RPC script",function(){assert.match(html,/lending-stage2-live-preflight-testnet\.js\?v=4/)});
  it("hands the temporary RPC directly to the approved deployment page",function(){assert.match(html,/id="continue" disabled/);assert.match(js,/lending-stage2-deploy-testnet\.html#rpc=/);assert.match(js,/encodeURIComponent\(privateRpc\)/)});
  ...
  it("does not persist RPC values",function(){assert.match(js,/pagehide/);assert.doesNotMatch(js,/localStorage|sessionStorage/)});
});
