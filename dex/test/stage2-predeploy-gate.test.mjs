...
describe("LQC Stage 2 predeployment gate", function () {
  it("runs role binding before deployment inputs, repository, audit, and live checks", function () {
    const calls = [];
    ...
    assert.deepEqual(calls.map(call => call.marker), ["reviewed", "reviewed", "reviewed", "reviewed", "reviewed"]);
    assert.deepEqual(calls.map(call => call.stdio), ["inherit", "inherit", "inherit", "inherit", "inherit"]);
  });

  it("fails closed and never runs later checks when role binding is blocked", function () {
    const calls = [];
    const status = runStage2PredeployGate({
      spawn(command, args) {
    ...
    assert.deepEqual(calls[0].args, ["run", "verify:role-review"]);
  });
  ...
  it("stops at the first later failure after readiness passes", function () {
    ...
    assert.deepEqual(calls[2].args,["run","gate:stage1"]);
  });
