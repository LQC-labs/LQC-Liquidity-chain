...
describe("Lending Stage-0 live preflight page",function(){
  const html=fs.readFileSync(new URL("../app/lending-stage0-live-preflight-testnet.html",import.meta.url),"utf8");
  const js=fs.readFileSync(new URL("../app/lending-stage0-live-preflight-testnet.js",import.meta.url),"utf8");
  it("checks exact four-feed CREATE state and bounded gas across two RPCs",function(){
    ...
    assert.match(js,/gasPrice=a\.gasPrice>b\.gasPrice/);assert.match(js,/total\*gasPrice\*120n\/100n/);
  });
  it("is read-only and keeps the private endpoint ephemeral",function(){
    ...
    assert.doesNotMatch(js,/localStorage|sessionStorage|window\.ethereum|eth_sendTransaction|eth_requestAccounts|privateKey|mnemonic/);
  });
