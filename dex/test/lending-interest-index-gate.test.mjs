...
describe("LQC official 4/5 Interest Rate / Index gate",function(){
 ...
 it("keeps borrow and supply indexes initialized in RAY units",function(){assert.match(idx,/RAY=1e27/);assert.match(idx,/initializeMarket/);assert.match(idx,/borrowIndexRay/);assert.match(idx,/supplyIndexRay/);});
 it("restricts index accrual mutation to the configured lending core",function(){assert.match(idx,/setCore/);assert.match(idx,/msg\.sender!=core/);});
