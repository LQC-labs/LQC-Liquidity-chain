...
describe("Router 2.0 Proof Gateway execution history page",function(){
  ...
  it("is read-only and requests no wallet account",function(){assert.match(html,/읽기 전용 · 거래 없음/);assert.doesNotMatch(js,/eth_requestAccounts|eth_sendTransaction|getSigner\(/);});
  ...
  it("rechecks every archived receipt and transaction on BSC Testnet",function(){assert.match(js,/new ethers\.BrowserProvider\(window\.ethereum\)/);assert.match(js,/chainId!==97n/);assert.match(js,/provider\.getTransactionReceipt\(entry\.transactionHash\)/);assert.match(js,/provider\.getTransaction\(entry\.transactionHash\)/);assert.match(js,/receipt\.blockHash\.toLowerCase\(\)!==final\.blockHash\.toLowerCase\(\)/);});
  it("requires fresh RPC finalized blocks reviewed code and consumed proof",function(){assert.match(js,/MAX_LATEST_AGE_MS=2\*60\*1000/);assert.match(js,/confirmations<MIN_CONFIRMATIONS/);assert.match(js,/ethers\.keccak256\(code\)!==GATEWAY_RUNTIME_HASH/);assert.match(js,/contract\.consumedProof\(entry\.execution\.proofHash\)/);assert.match(html,/현재 온체인 이력 재검증/);});
});
