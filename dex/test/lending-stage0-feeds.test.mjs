...
function signingProvider(options = {}) { return { getNetwork: async () => ({ chainId: 97n }), getBlockNumber: async () => 200, getBlock: async () => ({ hash: options.hash || blockHash, baseFeePerGas: options.baseFeePerGas ?? 3_000_000_000n }), getFeeData: async () => ({ gasPrice: options.gasPrice ?? 3_000_000_000n }), getTransactionCount: async () => options.nonce ?? 10, getBalance: async () => options.balance ?? ethers.parseEther("1"), getCode: async () => options.occupied ? "0x6001" : "0x", estimateGas: async () => options.gas ?? 100000n }; }
...
describe("Lending Stage-0 four-feed preparation", function () {
  ...
  it("uses the higher live gas price when BSC base fee is zero", async function () {
    ...
    assert.equal(result.conservativeGasPrice, "110000000"); assert.equal(result.totalApprovedGasLimit, "480000"); assert.equal(result.requiredGasBudget, "52800000000000");
  ...
  it("uses live gas price for signing when BSC base fee is zero", async function () {
    const c = await verificationContext(), review = buildLendingStage0DeploymentReview({ manifest: c.manifest, preflight: c.preflight }), packet = prepareLendingStage0ExecutionPacket({ manifest: c.manifest, preflight: c.preflight, review, approvedReviewDigest: review.reviewDigest }), result = await preflightLendingStage0Signing({ providers: [signingProvider({ baseFeePerGas: 0n, gasPrice: 100_000_000n }), signingProvider({ baseFeePerGas: 0n, gasPrice: 110_000_000n })], packet });
    assert.equal(result.conservativeGasPrice, "110000000"); assert.equal(result.requiredBalance, "52800000000000");
    await assert.rejects(preflightLendingStage0Signing({ providers: [signingProvider({ baseFeePerGas: 0n, gasPrice: 0n }), signingProvider({ baseFeePerGas: 0n, gasPrice: 0n })], packet }), /gas price missing/);
  });
