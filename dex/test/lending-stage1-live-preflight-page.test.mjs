...
describe("Lending Stage-1 live preflight page",function(){
  ...
  it("is read-only, cache-versioned and keeps the private endpoint ephemeral",function(){assert.match(html,/lending-stage1-live-preflight-testnet\.js\?v=2/);assert.match(html,/type="password"/);assert.match(js,/credentials:"omit"/);assert.match(js,/history\.replaceState/);assert.match(js,/pagehide/);assert.doesNotMatch(js,/localStorage|sessionStorage|window\.ethereum|eth_sendTransaction|eth_requestAccounts|privateKey|mnemonic/)});
