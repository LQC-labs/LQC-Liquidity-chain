...
describe('LQC BSC testnet MVP readiness gate',function(){
  ...
  it('returns all missing blockers in one run',function(){const report=collectMvpReadiness({},{}),ids=report.blockers.map(item=>item.id);for(const id of ['RPC_HTTPS','REVIEWED_CLEAN_COMMIT','RUNTIME_DEPLOYER_KEY','ROLE_REVIEW_FILE','DEPLOYER_ROLE_SEPARATION','FACTORY_OWNER','RISK_ADMIN','GUARDIAN_ADDRESS','TREASURY_ADDRESS','PANCAKE_V2_PIN','PANCAKE_V3_PINS','PANCAKE_V3_POOLS'])assert.ok(ids.includes(id));assert.ok(report.blockers.length>10)});
  ...
  it('blocks temporary role overrides before expensive checks',function(){const report=collectMvpReadiness({...valid,ALLOW_EOA_OWNER:'true'},{currentCommit:commit,dirty:false});assert.deepEqual(report.blockers.map(item=>item.id),['NO_ROLE_OVERRIDES'])});
