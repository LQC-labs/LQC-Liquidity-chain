...
const input=overrides=>({expectedStorage:true,keyMatches:true,newValue:'record',record:{state:'valid',value:{transactionHash:hash}},unverifiedTransactionHash:'',pendingRecoveryActive:false,...overrides});
...
describe('LQC cross-tab recovery synchronization',function(){
  ...
  it('releases a passive tab after another tab clears terminal recovery',function(){assert.equal(action(input({newValue:null,record:{state:'none'}})),'release')});
  it('retains recovery controls while this tab verifies its own transaction',function(){assert.equal(action(input({newValue:null,record:{state:'none'},unverifiedTransactionHash:hash})),'retain');assert.equal(action(input({newValue:null,record:{state:'none'},pendingRecoveryActive:true})),'retain')});
