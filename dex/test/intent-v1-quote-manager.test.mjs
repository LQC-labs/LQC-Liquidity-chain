...
const artifact=()=>JSON.parse(fs.readFileSync(new URL("../artifacts/contracts/intent-v1/LQCQuoteManager.sol/LQCQuoteManager.json",import.meta.url)));
...
describe("LQC Intent v1 Solver quote competition",function(){
  ...
  let provider,owner,guardian,outsider,solverA,solverB,walletA,walletB,manager,chainId,now,intentHash;

  beforeEach(async function(){
    provider=new ethers.BrowserProvider(ganache.provider({wallet:{mnemonic},chain:{chainId:31337},logging:{quiet:true}}));owner=await provider.getSigner(0);guardian=await provider.getSigner(1);outsider=await provider.getSigner(2);solverA=await provider.getSigner(3);solverB=await provider.getSigner(4);walletA=ethers.HDNodeWallet.fromPhrase(mnemonic,undefined,"m/44'/60'/0'/0/3");walletB=ethers.HDNodeWallet.fromPhrase(mnemonic,undefined,"m/44'/60'/0'/0/4");chainId=(await provider.getNetwork()).chainId;now=(await provider.getBlock("latest")).timestamp;intentHash=ethers.id("intent-quote-competition");const a=artifact();manager=await new ethers.ContractFactory(a.abi,a.bytecode,owner).deploy(await owner.getAddress(),await guardian.getAddress());await manager.waitForDeployment();await(await manager.setSolverAllowed(await solverA.getAddress(),true)).wait();await(await manager.setSolverAllowed(await solverB.getAddress(),true)).wait();
  });
