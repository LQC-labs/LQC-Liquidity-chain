...
describe("LQC minimal Lending core",function(){
  ...
  it("custodies collateral, supplies liquidity, borrows and repays principal",async function(){
    ...
    assert.equal(await core.debtOf(id,borrower.address),ethers.parseEther("90"));assert.equal(await debt.balanceOf(borrower.address),ethers.parseEther("91"));
  ...
  it("liquidates at most half the debt with the fixed five percent bonus",async function(){
    await(await core.connect(lender).supplyLiquidity(id,ethers.parseEther("500"),gas)).wait();await(await core.connect(borrower).depositCollateral(id,ethers.parseEther("2"),gas)).wait();await(await core.connect(borrower).borrow(id,ethers.parseEther("90"),borrower.address,gas)).wait();
    await(await cp.setAnswer(50_00000000n,gas)).wait();await(await cs.setAnswer(50_00000000n,gas)).wait();
    await(await engine.connect(other).liquidate(id,borrower.address,ethers.parseEther("90"),other.address,gas)).wait();
    const remaining=await core.debtOf(id,borrower.address),seized=await collateral.balanceOf(other.address);
    assert.ok(remaining>=ethers.parseEther("45"));assert.ok(seized>=ethers.parseEther("0.954"));assert.ok(seized<ethers.parseEther("0.955"));
  });

  it("caps seizure at available collateral and records residual bad debt",async function(){
    await(await core.connect(lender).supplyLiquidity(id,ethers.parseEther("500"),gas)).wait();await(await core.connect(borrower).depositCollateral(id,ethers.parseEther("2"),gas)).wait();await(await core.connect(borrower).borrow(id,ethers.parseEther("90"),borrower.address,gas)).wait();
    await(await cp.setAnswer(10_00000000n,gas)).wait();await(await cs.setAnswer(10_00000000n,gas)).wait();
    await(await engine.connect(other).liquidate(id,borrower.address,ethers.parseEther("90"),other.address,gas)).wait();
    assert.equal(await core.collateralOf(id,borrower.address),0n);assert.ok(await core.badDebtOf(id,borrower.address)>0n);assert.equal(await collateral.balanceOf(other.address),ethers.parseEther("2"));
  });

  it("rejects liquidation of a healthy account and direct core access",async function(){
    await(await core.connect(lender).supplyLiquidity(id,ethers.parseEther("500"),gas)).wait();await(await core.connect(borrower).depositCollateral(id,ethers.parseEther("2"),gas)).wait();await(await core.connect(borrower).borrow(id,ethers.parseEther("50"),borrower.address,gas)).wait();
