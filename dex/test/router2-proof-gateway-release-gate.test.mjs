...
describe("Router 2.0 Proof Gateway release safety gate",function(){
  ...
  it("matches compiled Proof and Gateway artifacts to the deployment bundle",function(){const hashes=assertArtifactHashes(new URL("..",import.meta.url).pathname);assert.match(hashes.proofRuntime,/^0x[0-9a-f]{64}$/);assert.match(hashes.gatewayRuntime,/^0x[0-9a-f]{64}$/);});
