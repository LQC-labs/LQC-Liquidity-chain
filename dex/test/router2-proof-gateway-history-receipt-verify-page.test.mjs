...
describe("Router 2.0 Proof Gateway history receipt independent verifier",function(){
  ...
  it("checks the claimed block remains canonical without signing",function(){assert.match(js,/provider\.getBlock\(receipt\.latestBlockNumber\)/);assert.match(js,/block\.hash\.toLowerCase\(\)!==receipt\.latestBlockHash\.toLowerCase\(\)/);assert.match(js,/receiptConfirmations<12/);});
  it("rejects stale RPC state and rechecks every archived transaction",function(){assert.match(js,/MAX_LATEST_AGE_MS=2\*60\*1000/);assert.match(js,/for\(const entry of verifiedEntries\)/);assert.match(js,/provider\.getTransactionReceipt\(entry\.transactionHash\)/);assert.match(js,/provider\.getTransaction\(entry\.transactionHash\)/);});
  it("pins signer gateway runtime and consumed proof for every trade",function(){assert.match(js,/transaction\.from\.toLowerCase\(\)!==SIGNER/);assert.match(js,/ethers\.keccak256\(code\)!==GATEWAY_RUNTIME_HASH/);assert.match(js,/contract\.consumedProof\(entry\.execution\.proofHash\)/);assert.doesNotMatch(js,/eth_requestAccounts|eth_sendTransaction|getSigner\(/);});
});
