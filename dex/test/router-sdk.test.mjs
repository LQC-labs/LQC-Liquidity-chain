...
describe("LQC Router browser SDK", function () {
  ...
  it("requires approval only when the selected spender allowance is insufficient", function () {
    ...
    assert.equal(sdk.hasSufficientNativeBalance(1_000n, 600n, 400n), true);
    assert.equal(sdk.hasSufficientNativeBalance(999n, 600n, 400n), false);
    assert.equal(sdk.hasSufficientNativeBalance(400n, 0n, 400n), true);
    assert.throws(() => sdk.hasSufficientNativeBalance(-1n, 0n, 1n));
  ...
  it("detects execution-route changes while a token approval is pending", function () {
    const single = { kind: "single", single: { best: { dexId: "0xA1", adapter: tokenA }, routeData: "0x1234" } };
    assert.equal(sdk.executionPlanFingerprint(single), `single:0xa1:${tokenA}:0x1234`);
    assert.notEqual(
      sdk.executionPlanFingerprint(single),
      sdk.executionPlanFingerprint({ kind: "single", single: { best: { dexId: "0xB2", adapter: tokenA }, routeData: "0x1234" } })
    );
    const split = {
      kind: "split",
      split: { dexIds: ["0xA1", "0xB2", "0xC3"], adapters: [tokenA, tokenB, tokenC], amountsIn: [60n, 0n, 40n] },
      routes: ["0x11", "0x22", "0x33"]
    };
    assert.equal(sdk.executionPlanFingerprint(split), `split:0xa1:${tokenA}:60:0x11|0xc3:${tokenC}:40:0x33`);
    assert.notEqual(
      sdk.executionPlanFingerprint(split),
      sdk.executionPlanFingerprint({ ...split, split: { ...split.split, adapters: [tokenB, tokenB, tokenC] } })
    );
