(function(root){
  ...
  function create({storage,key,deploymentFingerprint,validatePayload=()=>true,now=Date.now}){
    ...
    function reserve(anchorQuote,settlementContext){const value={version:1,deploymentFingerprint,anchorQuote,settlementContext,reservedAt:now()};if(!validatePayload(value))throw new Error('InvalidApprovalReservation');const serialized=JSON.stringify(jsonSafe(value));try{const current=state();if(current.state!=='none')throw new Error('PendingApprovalReservationConflict');storage.setItem(key,serialized);if(storage.getItem(key)!==serialized)throw new Error('PendingApprovalReservationUnavailable');return serialized}catch(error){if(error?.message==='PendingApprovalReservationConflict'||error?.message==='InvalidApprovalReservation')throw error;throw new Error('PendingApprovalReservationUnavailable')}}
    function clear(serialized){if(typeof serialized!=='string'||storage.getItem(key)!==serialized)return false;storage.removeItem(key);return storage.getItem(key)===null}
