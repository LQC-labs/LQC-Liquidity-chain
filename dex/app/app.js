(function(){
  'use strict';
  const cfg=window.LQC_FLOW_CONFIG,{ethers,LQCRouterSDK:sdk,LQCCandleData:candleData}=window,$=id=>document.getElementById(id);
  ...
  async function loadChartHistory(){
    ...
    try{
      const params={chainId:cfg.chainId,base:address(asset),quote:address(quote)},[candles,stats]=await Promise.all([candleData.load(cfg.candleDataUrl,{...params,timeframe,limit:120}),candleData.load(cfg.candleDataUrl,{...params,timeframe:'1h',limit:26})]);
  ...
  function toggleIndicator(button){
    const visible=button.getAttribute('aria-pressed')!=='true';button.setAttribute('aria-pressed',String(visible));button.classList.toggle('active',visible);
    if(button.dataset.indicator==='volume')$('volumeIndicator').classList.toggle('indicator-hidden',!visible);
    else if(button.dataset.indicator==='ma'){document.querySelector('.ma-legend').classList.toggle('indicator-hidden',!visible);[5,10,20].forEach(period=>$(`ma${period}Line`).classList.toggle('indicator-hidden',!visible))}
  }
  ...
  if(deployed)status('컨트랙트가 설정되었습니다. 지갑을 연결하세요.');render();tokenList();marketList();loadChartHistory();setMode('buy',false);if(localStorage.getItem(walletMemoryKey)==='1')setTimeout(()=>{const remembered=wallets[0]?.provider||window.ethereum;if(remembered)connect(remembered,false)},100);
})();
