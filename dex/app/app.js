(function(){
  ...
  function tokenList(query=''){const term=String(query).trim().toLowerCase(),tokens=cfg.tokens.filter(token=>!term||token.symbol.toLowerCase().includes(term)||token.name.toLowerCase().includes(term));ui.tokenList.replaceChildren(...tokens.map(token=>{const b=document.createElement('button');b.type='button';b.className='token-option';b.innerHTML=`<span class="token-symbol ${token.symbol==='LQC'?'mint':''}">${token.symbol.slice(0,4)}</span><b>${token.symbol}</b><span>${token.name}</span>`;b.onclick=()=>{const other=side==='in'?tokenOut:tokenIn;if(address(token)&&address(token)===address(other))return status(t('incompatibleNativeTokens'),'error');if(side==='in')tokenIn=token;else tokenOut=token;ui.dialog.close();mode=tokenIn.symbol==='LQC'?'sell':'buy';ui.title.textContent=t(mode==='buy'?'buyLqc':'sellLqc');ui.execute.textContent=ui.title.textContent;ui.execute.className=`execute-button ${mode}`;render();balances();quoteSoon()};return b}))}
