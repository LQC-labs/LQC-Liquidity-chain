(function(){
  ...
  const readProviders=deployed?cfg.rpcUrls.map(url=>new ethers.JsonRpcProvider(url,cfg.chainId,{staticNetwork:true})):[],readProvider=readProviders[0]||null,marketQuoteRouter=deployed?new ethers.Contract(cfg.quoteRouterAddress,quoteRouterAbi,readProvider):null;
  ...
  async function chainHeadWithin(timeoutMs=2500){
    const now=Date.now(),deadline=performance.now()+timeoutMs,probe=promise=>new Promise(resolve=>{const remaining=Math.max(0,deadline-performance.now()),timeout=setTimeout(()=>resolve(null),remaining);promise.then(value=>resolve(value),()=>resolve(null)).finally(()=>clearTimeout(timeout))}),active=readProviders.map((provider,index)=>({provider,index})).filter(item=>rpcSourceHealth[item.index].quarantinedUntil<=now),observations=await Promise.all(active.map(async item=>({...item,head:await probe(item.provider.getBlockNumber())}))),heads=observations.map(item=>item.head).filter(Number.isSafeInteger),headConsensus=chartHealth.consensusHead(heads,readProviders.length);if(!headConsensus){observations.filter(item=>!Number.isSafeInteger(item.head)).forEach(item=>recordRpcSource(item.index,false,now));return null}
    const blockObservations=await Promise.all(observations.filter(item=>Number.isSafeInteger(item.head)&&item.head>=headConsensus.head).map(async item=>({...item,block:await probe(item.provider.getBlock(headConsensus.head))}))),hashes=blockObservations.map(item=>item.block?.hash).filter(hash=>typeof hash==='string'&&/^0x[0-9a-fA-F]{64}$/.test(hash)),hashConsensus=chartHealth.consensusHash(hashes,readProviders.length);if(!hashConsensus){blockObservations.filter(item=>!item.block?.hash).forEach(item=>recordRpcSource(item.index,false,now));return null}
    for(const item of observations){const block=blockObservations.find(value=>value.index===item.index),healthy=item.head>=headConsensus.head&&item.head<=headConsensus.head+3&&block?.block?.hash?.toLowerCase()===hashConsensus.blockHash;recordRpcSource(item.index,healthy,now)}
