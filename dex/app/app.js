(function(){
  ...
  async function chainHeadWithin(timeoutMs=2500){
    const probe=provider=>new Promise(resolve=>{const timeout=setTimeout(()=>resolve(null),timeoutMs);provider.getBlockNumber().then(head=>resolve(head),()=>resolve(null)).finally(()=>clearTimeout(timeout))}),heads=(await Promise.all(readProviders.map(probe))).filter(Number.isSafeInteger);return chartHealth.consensusHead(heads,readProviders.length);
  }
