(function(){
  ...
  const status=(m,type='',hash='')=>{ui.status.className=`status ${type}`.trim();ui.statusText.textContent=m;if(hash){const link=document.createElement('a');link.className='tx-link';link.href=`${cfg.blockExplorerUrls[0]}/tx/${hash}`;link.target='_blank';link.rel='noopener noreferrer';link.textContent=t('viewTransaction');link.title=hash;ui.statusText.append(' · ',link)}};
  ...
  const createReadProvider=()=>new ethers.FallbackProvider(cfg.rpcUrls.map((url,index)=>({provider:new ethers.JsonRpcProvider(url,cfg.chainId,{staticNetwork:true}),priority:index+1,stallTimeout:1500,weight:1})),cfg.chainId,{quorum:1});
  ...
  async function swap(){
    ...
    try{
      await validateWalletExecutionSession(tradeAccount);disabled(true);const value=parseTradeAmount(raw,tokenIn),path=[address(tokenIn),address(tokenOut)];let plan=minimalMode?await validatedMinimalExecutionPlan(value,path):await validatedExecutionPlan(value,path),deadline=Math.floor(Date.now()/1000)+1200,bps=Math.round(validatedSlippage()*100);
      if(tokenIn.address!=='native'){let spender=executionSpender(plan),token=new ethers.Contract(tokenIn.address,tokenAbi,tradeSigner),allowance=await token.allowance(tradeAccount,spender);if(sdk.requiresTokenApproval(allowance,value)){status(t('approveToken',{token:tokenIn.symbol}));await approveExact(token,spender,allowance,value,tradeAccount);status(t('approvalComplete'));if(minimalMode)plan=await validatedMinimalExecutionPlan(value,path);else{plan=await validatedExecutionPlan(value,path);const refreshedSpender=executionSpender(plan);if(refreshedSpender.toLowerCase()!==spender.toLowerCase()){spender=refreshedSpender;allowance=await token.allowance(tradeAccount,spender);if(sdk.requiresTokenApproval(allowance,value)){status(t('newApprovalRequired'));await approveExact(token,spender,allowance,value,tradeAccount);plan=await validatedExecutionPlan(value,path);if(executionSpender(plan).toLowerCase()!==spender.toLowerCase())throw new Error('RouteChangedDuringApproval')}}}}}
  ...
  function tokenList(query=''){const term=String(query).trim().toLowerCase(),tokens=cfg.tokens.filter(token=>!term||token.symbol.toLowerCase().includes(term)||token.name.toLowerCase().includes(term)||String(token.address||'').toLowerCase().includes(term));ui.tokenList.replaceChildren(...tokens.map(token=>{const b=document.createElement('button');b.type='button';b.className='token-option';b.innerHTML=`<span class="token-symbol ${token.symbol==='LQC'?'mint':''}">${token.symbol.slice(0,4)}</span><span class="token-identity"><b>${token.symbol}</b><small>${token.name}</small></span><span class="token-verified" title="${t('verifiedToken')}" aria-label="${t('verifiedToken')}">✓ ${t('verifiedToken')}</span>`;b.onclick=()=>{const other=side==='in'?tokenOut:tokenIn;if(address(token)&&address(token)===address(other))return status(t('incompatibleNativeTokens'),'error');if(side==='in')tokenIn=token;else tokenOut=token;ui.dialog.close();mode=tokenIn.symbol==='LQC'?'sell':'buy';ui.title.textContent=t(mode==='buy'?'buyLqc':'sellLqc');ui.execute.textContent=ui.title.textContent;ui.execute.className=`execute-button ${mode}`;render();balances();quoteSoon()};return b}))}
