(function(){
  ...
  async function buildExecutionTransaction(plan,value,path,deadline,bps,recipient){
    ...
    if(tokenIn.address==='native'){const min=sdk.minimumAmountOut(plan.single.best.amountOut,ui.slippage.value);return nativeRouter.swapExactNativeForToken.populateTransaction(plan.single.best.dexId,path[1],min,recipient,deadline,plan.single.routeData,{value})}
    if(tokenOut.address==='native'){const min=sdk.minimumAmountOut(plan.single.best.amountOut,ui.slippage.value);return nativeRouter.swapExactTokenForNative.populateTransaction(plan.single.best.dexId,path[0],value,min,recipient,deadline,plan.single.routeData)}
    ...
    const min=sdk.minimumAmountOut(plan.single.best.amountOut,ui.slippage.value);return executionRouter.swapExactInput.populateTransaction(plan.single.best.dexId,path[0],path[1],value,min,recipient,deadline,plan.single.routeData);
  }
