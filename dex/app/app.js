(function(){
  ...
  async function blockIfPendingSwap(){const key=pendingSwapKey(account),raw=localStorage.getItem(key);if(!raw)return false;pendingGuardActive=true;let pending;try{pending=JSON.parse(raw)}catch{clearPendingSwap(account);return false}if(!ethers.isHexString(pending?.hash,32)){clearPendingSwap(account);return false}try{const receipt=await provider.getTransactionReceipt(pending.hash);if(receipt){clearPendingSwap(account);return false}}catch{status(t('pendingCheckFailed'),'error');return true}status(t('pendingTransaction'),'error',pending.hash);return true}
  async function blockIfNetworkTransactionPending(){try{const[latestNonce,pendingNonce]=await Promise.all([provider.getTransactionCount(account,'latest'),provider.getTransactionCount(account,'pending')]);if(pendingNonce>latestNonce){pendingGuardActive=true;status(t('networkPendingTransaction'),'error');return true}pendingGuardActive=false;return false}catch{pendingGuardActive=true;status(t('pendingCheckFailed'),'error');return true}}
  ...
  async function nativeMaxReserve(){const feeData=await provider.getFeeData(),gasPrice=feeData.maxFeePerGas||feeData.gasPrice;if(!gasPrice)throw new Error('GasPriceUnavailable');return BigInt(cfg.minimalRouterGasUnits||180000)*gasPrice*120n/100n}
  const localizedError=e=>{const guidance=sdk.explainSwapError(e);return t(`error_${guidance.code}`)};
  const disabled=v=>ui.execute.disabled=v||pendingGuardActive;
  ...
  async function validateFunds(transaction,value){
    const[nativeBalance,estimatedGas,feeData,tokenBalance]=await Promise.all([provider.getBalance(account),provider.estimateGas({...transaction,from:account}),provider.getFeeData(),tokenIn.address==='native'?Promise.resolve(null):balance(tokenIn)]);
  ...
  async function approveExact(token,spender,allowance,value,expectedAccount){
    for(const amount of sdk.exactApprovalAmounts(allowance,value)){await validateWalletExecutionSession(expectedAccount);const nonce=await provider.getTransactionCount(expectedAccount,'pending'),transaction={...(await token.approve.populateTransaction(spender,amount)),nonce};await validateFunds(transaction,value);await simulateExecution(transaction);await validateWalletExecutionSession(expectedAccount);sdk.validatePendingNonce(nonce,await provider.getTransactionCount(expectedAccount,'pending'));const tx=await token.runner.sendTransaction(transaction),receipt=await tx.wait();sdk.validateSwapReceipt(receipt,tx.hash,ethers)}
  }
  ...
  async function swap(){
    ...
    try{
      ...
      await validateWalletExecutionSession(tradeAccount);deadline=Math.floor(Date.now()/1000)+1200;const nonce=await provider.getTransactionCount(tradeAccount,'pending'),executionTransaction={...(await buildExecutionTransaction(plan,value,path,deadline,bps,tradeAccount)),nonce};await validateFunds(executionTransaction,value);status(t('simulatingTrade'));await simulateExecution(executionTransaction);await validateWalletExecutionSession(tradeAccount);sdk.validatePendingNonce(nonce,await provider.getTransactionCount(tradeAccount,'pending'));status(t('simulationPassed'),'success');const tx=await submitExecution(executionTransaction,tradeSigner);rememberPendingSwap(tx);
      status(t(plan.kind==='split'?'splitSubmitted':'tradeSubmitted'),'pending',tx.hash);const receipt=await tx.wait(1,120000);if(!receipt){status(t('transactionStillPending'),'pending',tx.hash);return}sdk.validateSwapReceipt(receipt,tx.hash,ethers);clearPendingSwap(tradeAccount);status(t('tradeComplete',{side:t(mode)}),'success',tx.hash);ui.amountIn.value='';await balances();await quote();
    }catch(e){const base=localizedError(e),reason=e?.info?.error?.message||e?.error?.message||e?.reason||e?.shortMessage||e?.message,detail=minimalMode&&reason?` · ${reason}`:'';status(`${base}${detail}`,'error')}finally{swapInFlight=false;lockTradeControls(false)}
  }
