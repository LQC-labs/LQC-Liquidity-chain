(function(){
  ...
  let walletProvider,provider,signer,account,router,quoteRouter,executionRouter,nativeRouter,splitOptimizer,autoRouter,gasCostOracle,wallets=[],boundWallets=new WeakSet(),side='in',timer,quoteVersion=0,mode='buy',tokenIn=cfg.tokens[0],tokenOut=cfg.tokens[2];
  ...
  async function quote(requestVersion=++quoteVersion){
    ...
    }catch{if(!sdk.isLatestQuote(requestVersion,quoteVersion))return;ui.selectedDex.textContent='유효한 경로 없음';ui.alternativeRoute.textContent='대체 경로도 없음';ui.preflightState.textContent='실행 불가';ui.preflightState.className='preflight-warning';disabled(true);status('이 거래쌍의 유동성을 확인할 수 없습니다. 수량을 줄이거나 다른 토큰을 선택하세요.','error')}
  }
  async function swap(){
