(function(){
  ...
  async function chainHeadWithin(timeoutMs=2500){
    const deadline=performance.now()+timeoutMs,probe=promise=>new Promise(resolve=>{const remaining=Math.max(0,deadline-performance.now()),timeout=setTimeout(()=>resolve(null),remaining);promise.then(value=>resolve(value),()=>resolve(null)).finally(()=>clearTimeout(timeout))}),observations=await Promise.all(readProviders.map(async provider=>({provider,head:await probe(provider.getBlockNumber())}))),heads=observations.map(item=>item.head).filter(Number.isSafeInteger),headConsensus=chartHealth.consensusHead(heads,readProviders.length);if(!headConsensus)return null;
    const blocks=await Promise.all(observations.filter(item=>Number.isSafeInteger(item.head)&&item.head>=headConsensus.head).map(item=>probe(item.provider.getBlock(headConsensus.head)))),hashes=blocks.map(block=>block?.hash).filter(hash=>typeof hash==='string'&&/^0x[0-9a-fA-F]{64}$/.test(hash)),hashConsensus=chartHealth.consensusHash(hashes,readProviders.length);return hashConsensus?Object.freeze({...headConsensus,blockHash:hashConsensus.blockHash,healthySources:hashConsensus.healthySources}):null;
  }
  ...
  async function loadChartHistory(){
    ...
    try{
      ...
      const consensus=await chainHeadWithin();if(request!==chartRequest)return;const sync=consensus?chartHealth.chainSync(primary.finalizedBlock,consensus.head,cfg.candleFinalityBlocks):null,hashLabel=consensus?`${consensus.blockHash.slice(0,8)}…${consensus.blockHash.slice(-6)}`:'',syncText=sync?`${sync.label} · ${sync.lagBlocks}블록 · ${hashLabel} · RPC ${consensus.healthySources}/${consensus.configuredSources}`:'다중 RPC 블록 해시 합의 실패',finalLevel=!consensus?'delayed':sync?.level==='stale'?'interrupted':['catching-up','unverified'].includes(sync?.level)?'delayed':quality.level;if(quality.level==='healthy'&&sync?.level==='synced')lastHealthyByChart.set(chartKey,updatedAt);chartState(`${timeframe} ${quality.label} · ${latencyMs}ms · ${syncText} · 정상 갱신 ${lastHealthyByChart.get(chartKey)||'기록 없음'}`,finalLevel);
