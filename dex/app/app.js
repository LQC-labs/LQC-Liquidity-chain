(function(){
  ...
  function selectMarket(token){const fallback=preferredQuote(token);if(!fallback)return status('이 토큰에 승인된 기준 마켓이 없습니다.','error');selectedAsset=token;quoteToken=fallback;ui.marketDialog.close();setMode(mode,false);window.scrollTo({top:0,behavior:'smooth'})}
  ...
  function chart(closes=chartSeries['1m'],timeframe='1m'){const g=$('candles'),bars=$('volumeBars'),ns='http://www.w3.org/2000/svg';g.replaceChildren();bars.replaceChildren();$('chartDataBadge').textContent=`${timeframe} 예시 차트 · 실시간 히스토리 연동 전`;closes.forEach((close,i)=>{const open=i?closes[i-1]+((i%5)-2)*3:108,x=i*16+3,up=close<open,high=Math.max(50,Math.min(open,close)-5-(i%4)*3),low=Math.min(340,Math.max(open,close)+7+(i%3)*4),wick=document.createElementNS(ns,'line'),body=document.createElementNS(ns,'rect');wick.setAttribute('x1',x+5.5);wick.setAttribute('x2',x+5.5);wick.setAttribute('y1',high);wick.setAttribute('y2',low);wick.setAttribute('class',`wick ${up?'up':'down'}`);body.setAttribute('x',x);body.setAttribute('y',Math.min(open,close));body.setAttribute('width',11);body.setAttribute('height',Math.max(5,Math.abs(open-close)));body.setAttribute('rx','.5');body.setAttribute('class',`candle ${up?'up':'down'}`);g.append(wick,body);const bar=document.createElement('i');bar.style.height=`${Math.min(66,9+Math.abs(open-close)*.8+(i%5)*2)}px`;if(!up)bar.className='hot';bars.append(bar)})}
  function selectTimeframe(timeframe){if(!chartSeries[timeframe])return;document.querySelectorAll('.timeframes [data-timeframe]').forEach(button=>button.classList.toggle('active',button.dataset.timeframe===timeframe));chart(chartSeries[timeframe],timeframe)}
