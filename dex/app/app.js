(function(){
  ...
  function setMode(next,open=true){mode=next;[tokenIn,tokenOut]=next==='buy'?[quoteToken,selectedAsset]:[selectedAsset,quoteToken];const label=`${selectedAsset.symbol} ${next==='buy'?'매수':'매도'}`;ui.title.textContent=label;ui.execute.textContent=label;ui.execute.className=`execute-button ${next}`;for(const button of [ui.buy,ui.buyTab])button.classList.toggle('active',next==='buy');for(const button of [ui.sell,ui.sellTab])button.classList.toggle('active',next==='sell');if(open)ui.order.classList.add('open');render();balances();quoteSoon()}

  function marketList(query=''){
    const needle=query.trim().toLowerCase(),tokens=cfg.tokens.filter(t=>!needle||[t.symbol,t.name,t.address].some(value=>String(value).toLowerCase().includes(needle)));
    ui.marketCount.textContent=`${cfg.tokens.length}개 마켓`;ui.marketEmpty.hidden=tokens.length>0;
  ...
  function selectMarket(token){const fallback=cfg.tokens.find(t=>t.symbol==='USDT')||cfg.tokens.find(t=>t.symbol==='BNB');selectedAsset=token;quoteToken=token===fallback?(cfg.tokens.find(t=>t.symbol==='LQC')||cfg.tokens.find(t=>t.symbol==='WBNB')):fallback;if(!quoteToken||selectedAsset===quoteToken)return status('이 토큰에 사용할 기준 마켓이 없습니다.','error');ui.marketDialog.close();setMode(mode,false);window.scrollTo({top:0,behavior:'smooth'})}
  ...
  function tokenList(query=''){
    const term=query.trim().toLowerCase(),tokens=cfg.tokens.filter(t=>
      [t.symbol,t.name,t.address].some(value=>String(value||'').toLowerCase().includes(term)));
    ui.tokenCount.textContent=`${tokens.length}/${cfg.tokens.length}개 등록`;ui.tokenEmpty.hidden=tokens.length!==0;
