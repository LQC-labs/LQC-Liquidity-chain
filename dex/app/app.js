(function(){
  ...
  async function approveExact(token,spender,allowance,value,expectedAccount){
    for(const amount of sdk.exactApprovalAmounts(allowance,value)){await validateWalletExecutionSession(expectedAccount);const transaction=await token.approve.populateTransaction(spender,amount);await validateFunds(transaction,value);await simulateExecution(transaction);await validateWalletExecutionSession(expectedAccount);const tx=await token.runner.sendTransaction(transaction),receipt=await tx.wait();sdk.validateSwapReceipt(receipt,tx.hash,ethers)}
  }
  ...
  async function swap(){
    ...
    try{
      ...
      await validateWalletExecutionSession(tradeAccount);deadline=Math.floor(Date.now()/1000)+1200;const executionTransaction=await buildExecutionTransaction(plan,value,path,deadline,bps,tradeAccount);await validateFunds(executionTransaction,value);status(t('simulatingTrade'));await simulateExecution(executionTransaction);await validateWalletExecutionSession(tradeAccount);status(t('simulationPassed'),'success');const tx=await submitExecution(executionTransaction,tradeSigner);
