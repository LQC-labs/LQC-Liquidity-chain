(function(){
  ...
  async function swap(){
    if((minimalMode?!router:!executionRouter)||!account)return connect();const raw=ui.amountIn.value.trim();if(!raw||Number(raw)<=0)return status(t('enterAmount'),'error');try{validatedSlippage()}catch{return status(t('invalidSlippage'),'error')}
    ...
    try{
      await validateWalletExecutionSession(tradeAccount);disabled(true);const value=ethers.parseUnits(raw,tokenIn.decimals),path=[address(tokenIn),address(tokenOut)];let plan=minimalMode?await validatedMinimalExecutionPlan(value,path):await validatedExecutionPlan(value,path),deadline=Math.floor(Date.now()/1000)+1200,bps=Math.round(validatedSlippage()*100);
