(function(){
  ...
  async function quote(requestVersion=++quoteVersion){
    ...
    try{
      const path=[address(tokenIn),address(tokenOut)],value=ethers.parseUnits(raw,tokenIn.decimals),probe=value>1000n?value/1000n:1n,plan=await executionPlan(value,path),out=plan.kind==='split'?plan.split.totalAmountOut:plan.single.best.amountOut,[impact,feeData,blockNumber]=await Promise.all([planPriceImpact(value,out,probe,path,plan),provider.getFeeData(),provider.getBlockNumber()]),ranked=plan.ranked||await rankedRoutes(value,path,plan.routes,plan.costs),standardQuote=window.LQCQuoteApi.normalizeQuote({dexId:plan.kind==='split'?'split':plan.single.dex.id,dexName:plan.kind==='split'?'LQC Split':plan.single.dex.name,tokenIn:path[0],tokenOut:path.at(-1),amountInRaw:value,amountOutRaw:out},{slippageBps:Math.round(Number(ui.slippage.value)*100),now:Date.now()}),min=BigInt(standardQuote.minimumOutputRaw),gasDex=plan.kind==='split'?{gasUnits:plan.summary.reduce((total,item)=>total+Number(item.dex.gasUnits||220000),0)}:plan.single.dex,gasWei=sdk.estimatedGasWei(gasDex,feeData.gasPrice||0n,tokenIn.address==='native'||tokenOut.address==='native');if(!sdk.isLatestQuote(requestVersion,quoteVersion))return;quoteSnapshot={chainId:cfg.chainId,tokenIn:path[0].toLowerCase(),tokenOut:path.at(-1).toLowerCase(),amountIn:value,amountOut:out,minimumOut:min,blockNumber,quotedAt:Date.now()};
