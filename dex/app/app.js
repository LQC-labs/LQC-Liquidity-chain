(function(){
  ...
  async function loadChartHistory(){
    ...
    if(!cfg.candleDataUrl||!candleData)return chart(chartSeries[timeframe],timeframe);
    $('chartDataBadge').textContent=`${timeframe} 히스토리 불러오는 중`;
    try{
      const candles=await candleData.load(cfg.candleDataUrl,{chainId:cfg.chainId,base:address(asset),quote:address(quote),timeframe,limit:120});
      ...
      const first=candles[0].close,last=candles.at(-1).close,change=(last-first)/first*100;
      ui.tickerChange.textContent=`${change>=0?'+':''}${change.toFixed(2)}%`;
      ui.tickerChange.classList.toggle('negative',change<0);
