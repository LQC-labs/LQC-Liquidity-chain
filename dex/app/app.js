(function(){
  ...
  async function swap(){
    ...
    try{
      ...
      status(t(plan.kind==='split'?'splitSubmitted':'tradeSubmitted'),'pending',tx.hash);const receipt=await tx.wait();sdk.validateSwapReceipt(receipt,tx.hash,ethers);clearPendingSwap(tradeAccount);status(t('tradeComplete',{side:t(mode)}),'success',tx.hash);ui.amountIn.value='';await balances();await quote();
