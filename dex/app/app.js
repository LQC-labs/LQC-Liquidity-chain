(function(){
  ...
  let walletProvider,provider,signer,account,router,quoteRouter,executionRouter,nativeRouter,splitOptimizer,autoRouter,gasCostOracle,wallets=[],boundWallets=new WeakSet(),side='in',timer,quoteVersion=0,quoteSnapshot=null,mode='buy',tokenIn=cfg.tokens[0],tokenOut=cfg.tokens[2],deploymentReady=false;
  ...
  async function buildExecutionTransaction(plan,value,path,deadline,bps){
    if(minimalMode){const min=plan.minimumOut;if(tokenIn.address==='native')return router.swapExactBNBForTokens.populateTransaction(min,path,account,deadline,{value});if(tokenOut.address==='native')return router.swapExactTokensForBNB.populateTransaction(value,min,path,account,deadline);return router.swapExactTokensForTokens.populateTransaction(value,min,path,account,deadline)}
  ...
  async function swap(){
    ...
    try{
      disabled(true);const value=ethers.parseUnits(raw,tokenIn.decimals),path=[address(tokenIn),address(tokenOut)];let plan=minimalMode?await validatedMinimalExecutionPlan(value,path):await validatedExecutionPlan(value,path),deadline=Math.floor(Date.now()/1000)+1200,bps=Math.round(Number(ui.slippage.value)*100);
      if(tokenIn.address!=='native'){let spender=executionSpender(plan),token=new ethers.Contract(tokenIn.address,tokenAbi,signer),allowance=await token.allowance(account,spender);if(sdk.requiresTokenApproval(allowance,value)){status(t('approveToken',{token:tokenIn.symbol}));await approveExact(token,spender,allowance,value);status(t('approvalComplete'));if(minimalMode)plan=await validatedMinimalExecutionPlan(value,path);else{plan=await validatedExecutionPlan(value,path);const refreshedSpender=executionSpender(plan);if(refreshedSpender.toLowerCase()!==spender.toLowerCase()){spender=refreshedSpender;allowance=await token.allowance(account,spender);if(sdk.requiresTokenApproval(allowance,value)){status(t('newApprovalRequired'));await approveExact(token,spender,allowance,value);plan=await validatedExecutionPlan(value,path);if(executionSpender(plan).toLowerCase()!==spender.toLowerCase())throw new Error('RouteChangedDuringApproval')}}}}}
      deadline=Math.floor(Date.now()/1000)+1200;const executionTransaction=await buildExecutionTransaction(plan,value,path,deadline,bps);await validateFunds(executionTransaction,value);status(t('simulatingTrade'));await simulateExecution(executionTransaction);status(t('simulationPassed'),'success');const tx=await submitExecution(executionTransaction);
