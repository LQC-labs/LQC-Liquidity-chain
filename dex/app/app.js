(function(){
  ...
  let trustedReadProviderIndexes=[];
  ...
  async function readOnlyBestQuote(tokenA,tokenB,value,routes){if(!trustedReadProviderIndexes.length)await chainHeadWithin();if(!trustedReadProviderIndexes.length)throw new Error('Browser RPC quorum unavailable');let lastError;for(const index of trustedReadProviderIndexes){try{return await marketQuoteRouters[index].quoteBest(tokenA,tokenB,value,routes)}catch(error){lastError=error}}throw lastError||new Error('Quote Router unavailable')}
