(function(){
  ...
  async function chainHeadWithin(timeoutMs=2500){if(!readProvider)return null;let timeout;try{return await Promise.race([readProvider.getBlockNumber(),new Promise((_,reject)=>{timeout=setTimeout(()=>reject(new Error('Chain head timeout')),timeoutMs)})])}catch{return null}finally{clearTimeout(timeout)}}
  ...
  async function loadChartHistory(){
    ...
    try{
      ...
      const chainHead=await chainHeadWithin();if(request!==chartRequest)return;const sync=chainHead===null?null:chartHealth.chainSync(primary.finalizedBlock,chainHead,cfg.candleFinalityBlocks),syncText=sync?`${sync.label} · ${sync.lagBlocks}블록`:'온체인 확인 대기',finalLevel=sync?.level==='stale'?'interrupted':['catching-up','unverified'].includes(sync?.level)?'delayed':quality.level;if(quality.level==='healthy'&&sync?.level==='synced')lastHealthyByChart.set(chartKey,updatedAt);chartState(`${timeframe} ${quality.label} · ${latencyMs}ms · ${syncText} · 정상 갱신 ${lastHealthyByChart.get(chartKey)||'기록 없음'}`,finalLevel);
