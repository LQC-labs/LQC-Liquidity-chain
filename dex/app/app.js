(function(){
  ...
  async function quote(){
    ...
    const raw=ui.amountIn.value.trim();if(!raw||Number(raw)<=0||!deployed){ui.selectedDex.textContent=t('comparePending');ui.alternativeRoute.textContent=t('showAfterQuote');ui.preflightState.textContent=t('checkPending');return}
    try{
    ...
    }catch{ui.selectedDex.textContent=t('noValidRoute');ui.alternativeRoute.textContent=t('noFallbackRoute');ui.preflightState.textContent=t('unavailable');ui.preflightState.className='preflight-warning';disabled(true);status(t('liquidityUnavailable'),'error')}
  }
  async function swap(){
    ...
    }catch(e){status(localizedError(e),'error')}finally{disabled(!deployed)}
  }
  ...
  i18n.apply();if(deployed)status(t('contractsReady'));render();tokenList();chart();setMode('buy',false);if(localStorage.getItem(walletMemoryKey)==='1')setTimeout(()=>{const remembered=wallets[0]?.provider||window.ethereum;if(remembered)connect(remembered,false)},100);
})();
