(function(){
  ...
  const ui={connect:$('connectButton'),language:$('languageSelect'),settings:$('settingsButton'),settingsPanel:$('settingsPanel'),amountIn:$('amountIn'),amountOut:$('amountOut'),minimum:$('minimumReceived'),gas:$('estimatedGas'),impact:$('priceImpact'),split:$('splitRatio'),route:$('routeLabel'),selectedDex:$('selectedDex'),selectedPath:$('selectedPath'),routeStrategy:$('routeStrategy'),alternativeRoute:$('alternativeRoute'),preflightState:$('preflightState'),quoteUpdated:$('quoteUpdated'),quoteFreshness:$('quoteFreshness'),balanceIn:$('balanceIn'),balanceOut:$('balanceOut'),walletDialog:$('walletDialog'),walletList:$('walletList'),positionBalance:$('positionBalance'),portfolioValue:$('portfolioValue'),tokenInButton:$('tokenInButton'),tokenOutButton:$('tokenOutButton'),flip:$('flipButton'),max:$('maxButton'),buy:$('buyAction'),sell:$('sellAction'),quick:$('quickTrade'),order:$('orderPanel'),close:$('closeOrder'),title:$('orderTitle'),execute:$('executeButton'),status:$('statusBox'),statusText:$('statusText'),dialog:$('tokenDialog'),tokenList:$('tokenList'),slippage:$('slippageInput')};
  const walletMemoryKey='lqc-flow-wallet-connected';
  const quoteTimeoutMs=8000,quoteRefreshMs=60000;
  let walletProvider,provider,signer,account,router,quoteRouter,executionRouter,nativeRouter,splitOptimizer,autoRouter,gasCostOracle,wallets=[],boundWallets=new WeakSet(),side='in',timer,freshnessTimer,quoteVersion=0,quoteSnapshot=null,mode='buy',tokenIn=cfg.tokens[0],tokenOut=cfg.tokens[2],deploymentReady=false,swapInFlight=false;
  ...
  const disabled=v=>{ui.execute.disabled=v;if(!v&&quoteSnapshot)markQuoteFresh()};
  ...
  async function quote(requestVersion=++quoteVersion){
    clearTimeout(freshnessTimer);quoteSnapshot=null;disabled(true);ui.quoteUpdated.textContent='—';ui.quoteFreshness.textContent=t('quotePending');
    ...
    try{const quoteDeadline=Date.now()+quoteTimeoutMs;
      if(minimalMode){
  ...
  async function swap(){
    ...
    try{
      ...
      await validateWalletExecutionSession(tradeAccount);deadline=Math.floor(Date.now()/1000)+1200;const nonce=await provider.getTransactionCount(tradeAccount,'pending'),executionTransaction={...(await buildExecutionTransaction(plan,value,path,deadline,bps,tradeAccount)),nonce};await validateFunds(executionTransaction,value);status(t('simulatingTrade'));await simulateExecution(executionTransaction);await validateWalletExecutionSession(tradeAccount);sdk.validatePendingNonce(nonce,await provider.getTransactionCount(tradeAccount,'pending'));status(t('simulationPassed'),'success');const tx=await submitExecution(executionTransaction,tradeSigner);
      status(t(plan.kind==='split'?'splitSubmitted':'tradeSubmitted'));const receipt=await tx.wait();sdk.validateSwapReceipt(receipt,tx.hash,ethers);status(t('tradeComplete',{side:t(mode)}),'success');ui.amountIn.value='';await balances();await quote();
    }catch(e){status(localizedError(e),'error')}finally{swapInFlight=false;lockTradeControls(false)}
  }
  ...
  i18n.apply();if(configured)status(t('contractsReady'));render();tokenList();chart();setMode('buy',false);setInterval(()=>{if(!document.hidden&&deploymentReady&&ui.amountIn.value.trim())quote()},quoteRefreshMs);if(localStorage.getItem(walletMemoryKey)==='1')setTimeout(()=>{const remembered=wallets[0]?.provider||window.ethereum;if(remembered)connect(remembered,false)},100);
})();
