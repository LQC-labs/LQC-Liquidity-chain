(function(){
  ...
  let walletProvider,provider,signer,account,router,quoteRouter,executionRouter,nativeRouter,splitOptimizer,autoRouter,gasCostOracle,wallets=[],boundWallets=new WeakSet(),side='in',timer,quoteRequestId=0,mode='buy',tokenIn=cfg.tokens[0],tokenOut=cfg.tokens[2];
  ...
  async function assertGasReserve(owner,plan,nativeValue=0n,approvalCount=0){const gasUnits=(plan.kind==='split'?plan.summary.reduce((total,item)=>total+Number(item.dex.gasUnits||220000),0):Number(plan.single.dex.gasUnits||220000))+approvalCount*65000,[nativeBalance,feeData]=await Promise.all([provider.getBalance(owner),provider.getFeeData()]),requiredGas=sdk.estimatedGasWei({gasUnits},feeData.gasPrice||0n,tokenIn.address==='native'||tokenOut.address==='native');if(!sdk.hasSufficientNativeBalance(nativeBalance,nativeValue,requiredGas))throw new Error('InsufficientGasReserve')}
  function quoteSoon(){clearTimeout(timer);const requestId=++quoteRequestId;timer=setTimeout(()=>quote(requestId),300)}
  ...
  async function quote(requestId=++quoteRequestId){
  ...
  async function swap(){
    ...
    try{
      disabled(true);const value=ethers.parseUnits(raw,tokenIn.decimals),path=[address(tokenIn),address(tokenOut)],inputBalance=await balance(tokenIn);if(inputBalance===null||!sdk.hasSufficientInputBalance(inputBalance,value))throw new Error('InsufficientInputBalance');let plan=await executionPlan(value,path),deadline=Math.floor(Date.now()/1000)+1200,bps=Math.round(Number(ui.slippage.value)*100),tx;
      if(tokenIn.address==='native'){await assertExecutionContext(executionAccount);await assertGasReserve(executionAccount,plan,value);const min=sdk.minimumAmountOut(plan.single.best.amountOut,ui.slippage.value);tx=await nativeRouter.swapExactNativeForToken(plan.single.best.dexId,path[1],min,executionAccount,deadline,plan.single.routeData,{value})}
      else{const planFingerprint=sdk.executionPlanFingerprint(plan),approvedAmountOut=plan.kind==='split'?plan.split.totalAmountOut:plan.single.best.amountOut;let spender=tokenOut.address==='native'?cfg.nativeRouterAddress:plan.kind==='split'?cfg.autoRouterAddress:cfg.executionRouterAddress,token=new ethers.Contract(tokenIn.address,tokenAbi,signer),allowance=await token.allowance(executionAccount,spender),approvals=sdk.tokenApprovalSequence(allowance,value);await assertGasReserve(executionAccount,plan,0n,approvals.length);if(approvals.length){for(const approvalAmount of approvals){status(approvalAmount===0n?`${tokenIn.symbol} 기존 승인을 안전하게 초기화하세요.`:`${tokenIn.symbol} 사용 승인을 확인하세요.`);await(await token.approve(spender,approvalAmount)).wait()}const confirmedAllowance=await token.allowance(executionAccount,spender);if(sdk.requiresTokenApproval(confirmedAllowance,value))throw new Error('ApprovalIncomplete');status('승인 완료 · 최신 가격과 경로를 다시 확인합니다.');plan=await executionPlan(value,path);const refreshedSpender=tokenOut.address==='native'?cfg.nativeRouterAddress:plan.kind==='split'?cfg.autoRouterAddress:cfg.executionRouterAddress,refreshedFingerprint=sdk.executionPlanFingerprint(plan),refreshedAmountOut=plan.kind==='split'?plan.split.totalAmountOut:plan.single.best.amountOut;if(refreshedSpender.toLowerCase()!==spender.toLowerCase()||!planFingerprint||refreshedFingerprint!==planFingerprint)throw new Error('RouteChangedDuringApproval');if(!sdk.isRefreshedOutputAcceptable(approvedAmountOut,refreshedAmountOut,ui.slippage.value))throw new Error('QuoteWorsenedDuringApproval');await assertGasReserve(executionAccount,plan)}await assertExecutionContext(executionAccount);deadline=Math.floor(Date.now()/1000)+1200;if(tokenOut.address==='native'){const min=sdk.minimumAmountOut(plan.single.best.amountOut,ui.slippage.value);tx=await nativeRouter.swapExactTokenForNative(plan.single.best.dexId,path[0],value,min,executionAccount,deadline,plan.single.routeData)}else if(plan.kind==='split')tx=await autoRouter.swapOptimizedExactInput(path[0],path[1],value,executionAccount,deadline,plan.routes,plan.costs,10,bps);else{const min=sdk.minimumAmountOut(plan.single.best.amountOut,ui.slippage.value);tx=await executionRouter.swapExactInput(plan.single.best.dexId,path[0],path[1],value,min,executionAccount,deadline,plan.single.routeData)}}
