(function(root){
  ...
  function normalizeTransaction(request){
    if(!request||!Number.isSafeInteger(request.chainId)||request.chainId<1||typeof request.to!=='string'||!/^0x[0-9a-fA-F]{40}$/.test(request.to)||typeof request.data!=='string'||!/^0x(?:[0-9a-fA-F]{2}){4,}$/.test(request.data)||typeof request.value!=='bigint'||request.value<0n||typeof request.gasLimit!=='bigint'||request.gasLimit<=0n)throw new Error('Chart wallet transaction input is invalid');
    return Object.freeze({chainId:request.chainId,to:request.to.toLowerCase(),data:request.data.toLowerCase(),value:request.value,gasLimit:request.gasLimit});
  }
  ...
  function transactionMatches(bound,request){try{const expected=bound?.transaction,actual=normalizeTransaction(request);return Boolean(expected)&&expected.chainId===actual.chainId&&expected.to===actual.to&&expected.data===actual.data&&expected.value===actual.value&&expected.gasLimit===actual.gasLimit}catch{return false}}
