(function(global){
  ...
  function buildSameChainIntentReceipt(input,ethers){
    const{chainId,intentHash,executionHash,transactionHash,blockHash,blockNumber,settledAt,hub,solver,router,dexId,tokenIn,tokenOut,recipient,amountIn,minimumAmountOut,actualAmountOut,gasUsed,effectiveGasPrice,gasEvidence,priceImpactEvidence,marketDeviationEvidence}=input||{};
  ...
  function verifySameChainIntentReceipt(receipt,ethers){
    if(!receipt||!ethers||receipt.version!==1||receipt.type!=='LQC_SAME_CHAIN_INTENT_RECEIPT'||!ethers.isHexString(receipt.receiptHash,32))return false;
