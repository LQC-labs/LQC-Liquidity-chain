(function(global){
  ...
  function buildExecutionEvidence(bound,settlement,deploymentFingerprint,completedAt,ethers){
    const request=bound?.request,execution=bound?.execution,transaction=bound?.transaction;if(!ethers||!bound||!request||!execution||!transaction||typeof deploymentFingerprint!=='string'||!ethers.isHexString(deploymentFingerprint,32)||!Number.isInteger(completedAt)||completedAt<=0||!settlement||!ethers.isHexString(settlement.transactionHash,32)||!ethers.isHexString(settlement.blockHash,32)||!Number.isInteger(settlement.blockNumber)||settlement.blockNumber<bound.blockNumber||!Number.isInteger(settlement.confirmations)||settlement.confirmations<1||typeof settlement.amountOut!=='bigint'||settlement.amountOut<execution.minimumAmountOut)throw new Error('Invalid execution evidence');
    const fee=transaction.type===2?{type:2,maxFeePerGas:transaction.maxFeePerGas.toString(),maxPriorityFeePerGas:transaction.maxPriorityFeePerGas.toString()}:{type:0,gasPrice:transaction.gasPrice.toString()},payload={version:1,type:'LQC_VERIFIED_SWAP',deploymentFingerprint:deploymentFingerprint.toLowerCase(),chainId:request.chainId,quote:{blockNumber:bound.blockNumber,blockHash:bound.blockHash,dexId:bound.dexId,adapter:bound.adapter,expectedAmountOut:bound.amountOut.toString(),expiresAt:bound.expiresAt},request:{router:request.router,tokenIn:request.tokenIn,tokenOut:request.tokenOut,amountIn:request.amountIn.toString(),routes:[...request.routes],slippageBps:request.slippageBps},execution:{router:execution.router,recipient:execution.recipient,minimumAmountOut:execution.minimumAmountOut.toString(),deadline:execution.deadline,kind:execution.kind},transaction:{to:transaction.to,dataHash:ethers.keccak256(transaction.data),value:transaction.value.toString(),gasLimit:transaction.gasLimit.toString(),nonce:transaction.nonce,...fee},settlement:{transactionHash:settlement.transactionHash.toLowerCase(),blockHash:settlement.blockHash.toLowerCase(),blockNumber:settlement.blockNumber,confirmations:settlement.confirmations,actualAmountOut:settlement.amountOut.toString()},completedAt};return Object.freeze({...payload,evidenceHash:ethers.keccak256(ethers.toUtf8Bytes(JSON.stringify(payload)))})
  }
  function verifyExecutionEvidence(evidence,deploymentFingerprint,ethers){try{if(!evidence||!ethers||!ethers.isHexString(evidence.evidenceHash,32)||typeof deploymentFingerprint!=='string'||evidence.deploymentFingerprint!==deploymentFingerprint.toLowerCase()||evidence.type!=='LQC_VERIFIED_SWAP'||evidence.version!==1||!ethers.isHexString(evidence.settlement?.transactionHash,32)||!ethers.isHexString(evidence.settlement?.blockHash,32)||BigInt(evidence.settlement.actualAmountOut)<BigInt(evidence.execution.minimumAmountOut)){return false}const{evidenceHash,...payload}=evidence;return ethers.keccak256(ethers.toUtf8Bytes(JSON.stringify(payload))).toLowerCase()===evidenceHash.toLowerCase()}catch{return false}}
  function explainSwapError(error){
    ...
    if(code==='ACTION_REJECTED'||message.includes('user rejected')||message.includes('user denied')||message.includes('transactionreplacementcancelled'))return{code:'USER_REJECTED',message:'지갑에서 거래가 취소되었습니다.',action:'원하시면 견적을 다시 확인한 뒤 재시도하세요.',retryable:true};
    if(message.includes('insufficienttradebalance'))return{code:'INSUFFICIENT_BALANCE',message:'거래에 사용할 토큰 잔액이 부족합니다.',action:'보유 수량을 확인하거나 거래 수량을 줄이세요.',retryable:true};
    if(code==='INSUFFICIENT_FUNDS'||message.includes('insufficient funds')||message.includes('insufficientgasbalance'))return{code:'INSUFFICIENT_GAS',message:'거래를 실행할 BNB 가스비가 부족합니다.',action:'소량의 tBNB를 준비한 뒤 다시 시도하세요.',retryable:true};
    if(message.includes('insufficientoutput')||message.includes('too little received')||message.includes('slippage')||message.includes('expiredbeforesigning'))return{code:'PRICE_MOVED',message:'가격이 변해 최소 수령 조건을 충족하지 못했습니다.',action:'새 견적을 받은 뒤 슬리피지를 확인하고 재시도하세요.',retryable:true};
    ...
    if(message.includes('pendingexecutionstorageunavailable'))return{code:'STORAGE_UNAVAILABLE',message:'거래 복구 기록을 안전하게 저장할 수 없습니다.',action:'브라우저 저장 공간과 개인정보 보호 설정을 확인한 뒤 다시 시도하세요.',retryable:true};
