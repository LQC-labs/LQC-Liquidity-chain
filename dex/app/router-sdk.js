(function(global){
  ...
  function executionIntentTypedData(intent,verifyingContract,ethers){
    if(!intent||!ethers.isAddress(verifyingContract)||verifyingContract.toLowerCase()===ethers.ZeroAddress.toLowerCase()||!ethers.isAddress(intent.sender)||!ethers.isAddress(intent.target)||!ethers.isHexString(intent.proofHash,32)||!ethers.isHexString(intent.calldataHash,32)||!Number.isSafeInteger(intent.chainId)||intent.chainId<=0||!Number.isSafeInteger(intent.nonce)||intent.nonce<0||!Number.isSafeInteger(intent.deadline)||intent.deadline<=0)throw new Error('Invalid EIP-712 execution intent');
    let value;try{value=BigInt(intent.value)}catch{throw new Error('Invalid EIP-712 execution intent')}if(value<0n)throw new Error('Invalid EIP-712 execution intent');
    return{domain:{name:'LQC Execution Intent',version:'1',chainId:intent.chainId,verifyingContract:verifyingContract.toLowerCase()},types:{ExecutionIntent:[{name:'proofHash',type:'bytes32'},{name:'sender',type:'address'},{name:'target',type:'address'},{name:'calldataHash',type:'bytes32'},{name:'value',type:'uint256'},{name:'nonce',type:'uint256'},{name:'deadline',type:'uint256'}]},message:{proofHash:intent.proofHash,sender:intent.sender,target:intent.target,calldataHash:intent.calldataHash,value,nonce:BigInt(intent.nonce),deadline:BigInt(intent.deadline)}};
  }
  function executionIntentTypedDataHash(intent,verifyingContract,ethers){const typed=executionIntentTypedData(intent,verifyingContract,ethers);return ethers.TypedDataEncoder.hash(typed.domain,typed.types,typed.message);}
  function canonicalExecutionIntent(intent,verifyingContract,ethers){const typedDataHash=executionIntentTypedDataHash(intent,verifyingContract,ethers);return{...intent,intentHash:typedDataHash,typedDataHash,hashScheme:'EIP712_V1',verifyingContract:verifyingContract.toLowerCase()};}
  function verifyCanonicalExecutionIntent(intent,verifyingContract,ethers){if(!intent||intent.hashScheme!=='EIP712_V1'||String(intent.verifyingContract||'').toLowerCase()!==String(verifyingContract||'').toLowerCase()||!ethers.isHexString(intent.intentHash,32)||!ethers.isHexString(intent.typedDataHash,32)||intent.intentHash.toLowerCase()!==intent.typedDataHash.toLowerCase())return false;try{return executionIntentTypedDataHash(intent,verifyingContract,ethers).toLowerCase()===intent.intentHash.toLowerCase()}catch{return false}}
  function verifyExecutionIntentSignature(intent,verifyingContract,signature,ethers){if(!ethers.isHexString(signature,65))return false;try{const typed=executionIntentTypedData(intent,verifyingContract,ethers),recovered=ethers.verifyTypedData(typed.domain,typed.types,typed.message,signature);return recovered.toLowerCase()===intent.sender.toLowerCase()}catch{return false}}
  ...
  global.LQCRouterSDK=Object.freeze({encodeRoute,encodeRoutes,minimumAmountOut,priceImpactBps,priceImpactFromExpected,estimatedGasWei,routeFeeBps,summarizeSplit,isSplitNetBetter,walletSessionState,validateExecutionSession,validatePendingNonce,requiresTokenApproval,exactApprovalAmounts,isLatestQuote,validateExecutionQuote,rankRouteQuotes,buildBestExecutionProof,verifyBestExecutionProof,validateExecutionPlanProof,buildExecutionIntent,executionIntentTypedData,executionIntentTypedDataHash,canonicalExecutionIntent,verifyCanonicalExecutionIntent,verifyExecutionIntentSignature,verifyExecutionIntent,buildIntentBoundSettlementReceipt,verifyIntentBoundSettlementReceipt,buildQuoteApiRequest,validateQuoteApiResponse,recoveryActionForError,buildSettlementReceipt,verifySettlementReceipt,verifyCanonicalSettlement,verifyCanonicalNativeSettlement,explainSwapError,verifyUiDeployment,verifyMinimalUiDeployment,validateSwapReceipt,validateTransactionFunds,SUPPORTED_V3_FEES:[...SUPPORTED_V3_FEES]});
