...
function renderMarkets(filter = '') {
  ...
  for (const market of listActiveFuturesMarkets()) {
    ...
    const row = document.createElement('button'); row.type = 'button'; row.className = 'market' + (market.symbol === symbol ? ' selected' : '');
...
function refreshEstimate() {
  ...
  try { const order = buildDemoOrder(orderInput('LONG')); byId('margin').textContent = formatUsdt(order.initialMargin); }
  catch { byId('margin').textContent = '-'; }
}
...
function renderPositions() {
  ...
  for (const position of demoPositions) {
    ...
    const close = document.createElement('button'); close.type = 'button'; close.textContent = '데모 청산'; close.addEventListener('click', () => closePosition(position.id)); card.appendChild(close); root.appendChild(card);
  }
}

function closePosition(id) {
  const index = demoPositions.findIndex((position) => position.id === id); if (index < 0) return;
  const position = demoPositions[index]; const marked = markDemoPosition(position, prices[position.symbol]);
  availableBalance += position.collateral + marked.unrealizedPnl; demoPositions.splice(index, 1); renderBalance(); renderPositions();
  const sign = marked.unrealizedPnl >= 0 ? '+' : ''; byId('message').textContent = `데모 포지션 종료 · ${position.symbol} ${position.side} · PnL ${sign}${marked.unrealizedPnl.toFixed(4)} USDT`;
}

function submit(side) {
  try {
    if (byId('reduce').checked) throw new Error('REDUCE_ONLY_NOT_YET_SUPPORTED_IN_DEMO');
    const order = buildDemoOrder(orderInput(side)); if (order.initialMargin > availableBalance) throw new Error('INSUFFICIENT_DEMO_BALANCE');
    ...
    availableBalance -= position.collateral; demoPositions.push(position); renderBalance(); renderPositions();
    byId('message').textContent = `데모 포지션 생성 · ${position.symbol} ${position.side} · 격리 증거금 ${formatUsdt(position.collateral)}`;
...
byId('search').addEventListener('input', (event) => renderMarkets(event.target.value)); for (const id of ['qty', 'price', 'leverage']) byId(id).addEventListener('input', refreshEstimate);
byId('long').addEventListener('click', () => submit('LONG')); byId('short').addEventListener('click', () => submit('SHORT'));
renderMarkets(); refreshEstimate(); renderBalance(); renderPositions();
