// LQC Flow Futures — minimal versioned REST routing contract.
// Transport-independent DEMO adapter: a future Node/edge server can map an
// HTTP request into route({ method, path, query }) and serialize the result.
...
const STATUS_BY_CODE = Object.freeze({
  UNKNOWN_MARKET: 404,
  MARK_PRICE_UNAVAILABLE: 503,
  DEPTH_UNAVAILABLE: 503,
  TRADES_UNAVAILABLE: 503,
  KLINES_UNAVAILABLE: 503,
  STREAM_UNAVAILABLE: 503,
  INVALID_LIMIT: 400,
  INVALID_KLINE_INTERVAL: 400,
  METHOD_NOT_ALLOWED: 405,
  ROUTE_NOT_FOUND: 404
});

function requestId() {
  return `lqc-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
}

function response(status, body, id) {
  return Object.freeze({
    status,
    headers: Object.freeze({
      'content-type': 'application/json; charset=utf-8',
      'x-lqc-api-version': REST_API_VERSION,
      'x-request-id': id
    }),
...
function failure(error, id) {
  ...
  return response(status, {
    code,
    message: code === 'INTERNAL_ERROR' ? 'Internal server error.' : code,
    requestId: id,
    timestamp: Date.now()
  }, id);
}
...
function ok(data, id) {
  return response(200, { data, requestId: id, timestamp: Date.now() }, id);
}
...
// Production transport requirements to add outside this browser demo:
// - TLS termination and trusted proxy policy
// - IP/API-key rate limits
// - structured access/audit logs
// - CORS policy
// - metrics/tracing
// - authenticated routes with signed requests and replay protection
