...
export function createDemoPositionBook() {
  ...
  function add(fill) {
    ...
    if (existing) {
      const merged = mergeDemoPosition(existing, { symbol: fill.symbol, side: fill.side, quantity: fill.quantity, entryPrice: fill.entryPrice, collateral: fill.collateral });
      const next = Object.freeze({ ...merged, marginMode, takeProfit: fill.takeProfit ?? existing.takeProfit ?? null, stopLoss: fill.stopLoss ?? existing.stopLoss ?? null });
  ...
  function reduce(criteria, quantity) {
    const key = keyOf(criteria), current = positions.get(key);
  ...
  function evaluateLiquidation(criteria, markPrice) {
    const current = positions.get(keyOf(criteria));
  ...
  function liquidate(criteria, markPrice) {
    ...
    return Object.freeze({ liquidated: true, previous: current, position: null, decision, liquidatedAt: new Date().toISOString() });
  }

  function remove(criteria) {
    const key = keyOf(criteria), current = positions.get(key) || null;
  ...
  function removeCross(expected = null) {
    ...
    if (expected !== null) {
      ...
      const currentKeys = current.map(keyOf).sort(), expectedKeys = expected.map(keyOf).sort();
      if (currentKeys.length !== expectedKeys.length || currentKeys.some((key, index) => key !== expectedKeys[index])) throw new Error('CROSS_POSITION_SET_CHANGED');
    }
    for (const position of current) positions.delete(keyOf(position));
    return Object.freeze(current);
  }

  function restoreCross(snapshot) {
    if (!Array.isArray(snapshot)) throw new Error('INVALID_CROSS_POSITION_SNAPSHOT');
    const incoming = snapshot.map((position) => {
      if (!position || position.marginMode !== 'CROSS') throw new Error('INVALID_CROSS_POSITION_SNAPSHOT');
      return [keyOf(position), position];
    });
    if (incoming.some(([key]) => positions.has(key))) throw new Error('CROSS_POSITION_RESTORE_CONFLICT');
    for (const [key, position] of incoming) positions.set(key, position);
    return Object.freeze(snapshot.slice());
  }
