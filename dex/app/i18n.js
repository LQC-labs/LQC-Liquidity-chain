(function(root){
  ...
  function translate(key,values={},locale=current){const template=root.LQCLocales?.[locale]?.[key]??root.LQCLocales?.[fallback]?.[key]??key;return String(template).replace(/\{(\w+)\}/g,(_,name)=>values[name]??`{${name}}`)}
