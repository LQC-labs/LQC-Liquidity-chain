...
export async function preflightLendingStage0Feeds({ providers, manifest, deployer }) {
  ...
  const observations = await Promise.all(providers.map(async p => {
    const block = await p.getBlock(blockNumber); if (!block?.hash || !block.baseFeePerGas) throw new Error("Canonical block missing");
    const [nonce, balance] = await Promise.all([p.getTransactionCount(deployer, blockNumber), p.getBalance(deployer, blockNumber)]);
    ...
    return { blockHash: block.hash.toLowerCase(), baseFeePerGas: String(block.baseFeePerGas), nonce: String(nonce), balance: String(balance), addresses, gas: gas.map(String) };
  }));
  ...
  const totalGas = gas.reduce((a, b) => a + b, 0n), requiredGasBudget = totalGas * BigInt(first.baseFeePerGas) * 120n / 100n;
  if (BigInt(first.balance) < requiredGasBudget) throw new Error("Insufficient tBNB for four feed deployments");
  const result = { schemaVersion: 1, recordType: "LQC_LENDING_STAGE0_MULTI_RPC_PREFLIGHT", status: "PREFLIGHT_VERIFIED_FOR_REVIEW", network: manifest.network, manifestDigest, deployer: ethers.getAddress(deployer), blockNumber, blockHash: first.blockHash, deployerNonce: first.nonce, deployerBalance: first.balance, rpcCount: providers.length, deployments: manifest.orderedActions.map((a, i) => ({ id: a.id, asset: a.asset, role: a.role, predictedAddress: first.addresses[i], initialAnswer: a.initialAnswer, conservativeGas: gas[i].toString() })), totalConservativeGas: totalGas.toString(), requiredGasBudget: requiredGasBudget.toString(), transactionOccurred: false, safety: "Read-only preflight; no signing or transaction broadcast." };
