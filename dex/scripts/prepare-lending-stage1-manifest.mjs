...
async function main(){const[configFile,outputFile]=process.argv.slice(2);if(!configFile)throw new Error("Usage: node prepare-lending-stage1-manifest.mjs <config.json> [manifest.json]");const config=JSON.parse(fs.readFileSync(path.resolve(configFile),"utf8")),result=await buildLendingStage1Manifest(config);if(outputFile)fs.writeFileSync(path.resolve(outputFile),`${JSON.stringify(result,null,2)}\n`,{flag:"wx"});else console.log(JSON.stringify(result,null,2))}
