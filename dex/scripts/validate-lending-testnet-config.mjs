...
export function validateLendingTestnetConfig(config) {
  ...
  const baseIdentities=[governance,guardian,treasury,collateral,debt,...feeds];if(new Set(baseIdentities).size!==baseIdentities.length)throw new Error("Lending roles, assets and feeds must not alias");
  let composite,compositeCheck=[];if(config.composite!==undefined){const executor=address(config.composite?.executor,"Composite Executor"),core=address(config.composite?.lendingCore,"Lending Core"),adapter=address(config.composite?.supplyAdapter,"Supply Adapter");if(new Set([...baseIdentities,executor,core,adapter]).size!==baseIdentities.length+3)throw new Error("Lending roles, assets, feeds and contracts must not alias");if(String(config.composite.marketId).toLowerCase()!==marketId.toLowerCase())throw new Error("Composite Adapter market binding mismatch");composite={executor,lendingCore:core,supplyAdapter:adapter,marketId};compositeCheck=["market-pinned-composite-adapter"]}
  const body = {schemaVersion:1,recordType:"LQC_LENDING_TESTNET_CONFIG_PREFLIGHT",status:"PASS_LENDING_TESTNET_CONFIG_PREFLIGHT",network:{name:"BSC Testnet",chainId:97},roles:{governanceSafe:governance,guardianSafe:guardian,treasurySafe:treasury},market:{...config.market,collateralAsset:collateral,debtAsset:debt,marketId},oracle:{...config.oracle,feeds},rate:config.rate,...(composite?{composite}:{}),checks:["chain-97","safe-role-separation","governance-4-of-7","guardian-3-of-5","ltv-50-percent","liquidation-threshold-70-percent","liquidation-bonus-5-percent","bounded-caps","four-distinct-dual-feeds","bounded-interest-model",...compositeCheck],transactionOccurred:false,safety:"Offline configuration validation only. Composite binding is optional until its contracts exist. No RPC, wallet, key, signature, approval, deployment, token movement, or transaction."};
