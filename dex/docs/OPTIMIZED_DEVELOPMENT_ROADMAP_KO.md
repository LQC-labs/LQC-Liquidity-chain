...
| Lending | Oracle·격리 시장·Index 회계·청산·Bad Debt·경제 불변식·Composite 공급 영수증·42개 변경 진입점 감사 범위·독립 감사 인계·테스트넷 설정 사전검증 로컬 완료 | 실제 설정안 확정·제한형 테스트넷 배포 증거 |
...
**Composite Coordinator**와 **SettlementHub·FinalityVerifier·SolverRegistry 결합**의 로컬 구현은 완료했다. Lending·Composite Supply의 7개 계약과 42개 변경 진입점을 감사 범위로 고정했고, 소스·ABI·바이트코드·테스트 증거를 커밋에 결합하는 독립 감사 인계 패키지와 BSC 테스트넷 설정 사전검증기를 추가했다. 다음 저장소 작업은 **Lending 제한형 테스트넷 배포 manifest와 다중 RPC 배포 전 검증**이다. 실제 배포·Safe 승인은 운영 설정안 확정 후 별도 단계로 수행하며, Intent V1은 Stage 0 Bond 선택 증거부터 실제 테스트넷 경로를 재개한다.
