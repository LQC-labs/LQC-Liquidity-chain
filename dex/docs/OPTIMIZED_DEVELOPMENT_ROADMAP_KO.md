...
| Lending | Oracle·격리 시장·Index 회계·청산·Bad Debt 복구 로컬 구현 중 | 상태 기반 경제 테스트·Composite Adapter 구현 |
...
10. **Cross-chain 및 메인넷** — Bridge Registry와 인증 Adapter는 별도 범위로 구현·감사하고, 그 뒤 제한적 메인넷 출시를 검토한다.
...
**Composite Coordinator**와 **SettlementHub·FinalityVerifier·SolverRegistry 결합**의 로컬 구현은 완료했다. Lending은 청산 잔여 Bad Debt를 준비금 상계, Governance 자본 투입, 중지 시장의 누적 20% 이내 공급자 손실 순서로 복구하도록 제한했다. 다음 저장소 작업은 **상태 기반 경제 공격 테스트와 Composite Lending Adapter**다. 동시에 운영 승인이 가능해지면 **Intent V1 Stage 0 Bond 선택 증거**부터 실제 테스트넷 경로를 재개한다.
