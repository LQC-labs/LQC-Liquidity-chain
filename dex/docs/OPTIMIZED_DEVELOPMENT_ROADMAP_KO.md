...
| Lending | 미구현 | 최소 코어·오라클·청산·부채 한도부터 구현 |
...
**Composite Coordinator**와 **SettlementHub·FinalityVerifier·SolverRegistry 결합**의 로컬 구현은 완료했다. 다음 저장소 작업은 **최소 Lending 코어의 담보·차입·이자·Oracle·청산 경계 설계**다. 동시에 운영 승인이 가능해지면 **Intent V1 Stage 0 Bond 선택 증거**부터 실제 테스트넷 경로를 재개한다.
