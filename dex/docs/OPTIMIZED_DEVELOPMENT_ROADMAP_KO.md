...
- **10-12 저장소 완료:** 감사기관 기술·경제보안 경험 중심 100점 평가표, 자동 탈락 조건, 동일 범위 RFP, 견적·일정·재검증 비교 필드, 제출 체크리스트와 6개 별도 승인 경계를 불변 bundle로 고정한다. 초기 상태는 제안 0개·선정업체 없음·외부 전송 없음이다.
- **10-13 저장소 완료:** OpenZeppelin·ChainSecurity·Certora의 공식 서비스 페이지와 공개 Lending 감사 보고서만 근거로 후보 비교 bundle을 생성한다. 공개 근거로 확인 가능한 Lending·Oracle·방법론·수정검토 범위만 기록하고, 담당자·일정·가격·독립 재검증·이해충돌·범위 수락은 모두 미확인으로 유지하며 가중점수·후보선정·외부연락·자료전송을 금지한다.
- **10-14 저장소 완료:** Stage 12 RFP와 Stage 13의 3개 후보를 결합한 외부 전송 승인 게이트를 생성한다. 4-of-7 Governance 승인 기록·정확한 수신 후보·전송 자료 digest·만료시간·기밀성 검토가 없으면 연락과 자료 전송을 차단한다. 완전한 서면 제안서만 100점 평가표로 계산할 수 있고 평가 결과는 업체 선정을 수행하지 않는다.
- **10-15 저장소 완료:** 감사 전 보강으로 36개월 월별 이자 누적·반올림 완전상환, 단일 Oracle 급락 상태보존·이중 확인 후 청산, 다중 부실계정 Bad Debt 격리·담보 초과 압류 방지를 추가한다. 핵심 경제·이자·Lending 테스트 digest와 Stage 1~9 실제 테스트넷 실행 체크리스트를 결합하고, 9개 실행 증거가 모두 없는 현재 상태를 `코드 강화 완료·테스트넷 실행 대기`로 고정한다.
- **10-16 다음:** 실제 BSC 테스트넷 Stage 1부터 순서대로 배포·Safe 실행·제한형 파일럿을 수행하고 각 단계의 canonical 증거를 채운다. 지갑 서명과 트랜잭션은 단계별 사용자 승인 후 실행한다.
- **10-16 진행:** 실배포 사전점검에서 Stage 1 설정이 미래 Composite 주소를 요구하는 순환 의존성과 tLQC용 운영 가능 Oracle 부재를 확인했다. Composite 결합은 계약 배포 후 선택 검증하도록 분리하고, BSC 테스트넷 전용 가격 Feed는 Governance만 양수 가격을 갱신하며 2단계 소유권 이전을 사용하도록 추가한다. 실제 배포 전에는 4개 독립 Feed 주소와 2개 이상 RPC 검증이 필요하다.
- **10-16 Stage 0 저장소 완료:** tLQC 담보/WBNB 부채의 Primary·Secondary Feed 4개를 Governance 소유로 고정한다. RPC 상태진단, 다중 RPC CREATE 주소·가스 사전검증, 사람이 확인하는 승인 digest, 무서명 순차 실행 패킷, 서명 직전 nonce·주소·잔액·가스 재검증, canonical 배포 사후검증, 검증 주소의 설정 자동반영과 전체 완료 digest를 구현했다. 현재 공개 RPC는 한 곳만 안정적이므로 실제 배포는 독립 RPC 2개 확보와 별도 거래 승인 전까지 대기한다.
