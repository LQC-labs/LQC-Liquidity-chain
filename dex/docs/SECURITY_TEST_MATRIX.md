...
| Testnet Vault solvency and backing monitoring | `test/monitoring.test.mjs` | Strategy debt/cap, idle backing, adapter backing, insolvency, and pause states produce fail-closed monitoring results. The expected initial allocation pause is healthy only while it matches the deployment record; any drift is critical and produces a non-automatic Risk Multisig, evidence, review, Timelock, and post-check response. |
