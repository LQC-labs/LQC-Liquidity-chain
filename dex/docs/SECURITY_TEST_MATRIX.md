...
The current suite compiles **48 Solidity sources** and reports **178 passing tests**. Future commits may change that count; the CI result for the exact reviewed commit is authoritative.
...
| Testnet Vault solvency and backing monitoring | `test/monitoring.test.mjs` | Strategy debt/cap, idle backing, adapter backing, insolvency, and pause states produce fail-closed monitoring results. Backing breaches produce a non-automatic shutdown, evidence preservation, Strategy review, Timelock recovery, and post-check plan. The expected initial allocation pause is healthy only while it matches the deployment record; any drift is critical and produces its own governed response. |
