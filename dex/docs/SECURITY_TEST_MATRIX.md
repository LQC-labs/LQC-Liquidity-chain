...
| Testnet Vault solvency, limits, and backing monitoring | `test/monitoring.test.mjs` | Strategy debt/cap, idle backing, adapter backing, insolvency, and pause states produce fail-closed monitoring results. Backing breaches produce a non-automatic shutdown and governed recovery plan. Unapproved Strategy cap or loss-limit expansion is critical; safer reductions remain warnings until governance records the new baseline. Allocation-state drift produces its own Risk Multisig, evidence, review, Timelock, and post-check response. |
