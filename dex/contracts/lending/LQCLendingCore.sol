...
contract LQCLendingCore {
    ...
    error InsufficientLiquidity(); error InexactTransfer(); error Reentrancy(); error InsufficientReserves();
    ...
    function borrow(bytes32 id,uint256 amount,address receiver) external nonReentrant{
        if(amount==0)revert InvalidAmount();if(receiver==address(0))revert ZeroAddress();_accrue(id);if(amount>availableLiquidity(id))revert InsufficientLiquidity();MarketState storage state=marketStates[id];
    ...
    function repay(bytes32 id,uint256 amount,address account) external nonReentrant returns(uint256 repaid){
        ...
        _pullExact(config.debtAsset,msg.sender,repaid);debtSharesOf[id][account]=sharesHeld-shares;marketStates[id].totalDebtShares-=uint128(shares);emit Repaid(id,msg.sender,account,repaid,shares);
    }
